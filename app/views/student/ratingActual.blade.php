...
@section ('content')
				...
				<dl class="dl-horizontal">
					<dt>Carrera:</dt>
					<dd><strong>{{ $student->degree->name }}</strong> <span class="text-capitalize">{{ $student->degree->mode }}</span></dd>
					<dt>Grado:</dt>
					<dd><strong>{{ $student->lapse }}</dd>
						...
						<table class="table table-striped">
							...
							<tbody>
								@if ($subjects_lapse = Subject::where('degree_id', $student->degree->id)->where('lapse', $student->lapse)->get() )
					...
					@foreach(range(1, $student->degree->lapse) as $lapse_num)
						...
						<table class="table table-striped">
							...
							<tbody>
								...
								@if ($subject_name = $subject_lapse->name) @endif
								@if ($rating_p = Rating::where('profile_id', $student->id)->where('subject_id', $subject_lapse->id)->firstOrFail()) @endif
